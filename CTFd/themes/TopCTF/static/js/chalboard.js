...
function updateChalWindow(obj) {
    $.get(script_root + '/themes/TopCTF/static/js/templates/challenges/'+obj.type+'/'+obj.type+'-challenge-modal.hbs', function(template_data){
        $('#chal-window').empty();
        ...
        $.getScript(script_root + '/themes/TopCTF/static/js/templates/challenges/'+obj.type+'/'+obj.type+'-challenge-script.js',
            function() {
                // Handle Solves tab
    ...
    $.get(script_root + "/chals", function (data) {
        ...
        for (var i = challenges['game'].length - 1; i >= 0; i--) {
            ...
            if ($.inArray(challenges['game'][i].category, categories) == -1) {
                ...
                //         '</div>' +
                //     '</div>');
                // categoryrow.find(".category-header").append($("<h3>"+ category +"1111"+"</h3>"));
                let html = '';
                html+='<li>';
                html+='<a href="#'+category+'"  data-toggle="tab">'+category+'</a>';
                html+='</li>';
                $('#myTab').append(html);
                let tabContentHtml='';
                tabContentHtml+='<div style="overflow: hidden;" class="tab-pane fade " id="'+category+'"  data-id="{0}-row">'.format(categoryid);
	            tabContentHtml+='</div>';
        ...
        for (var i = 0; i <= challenges['game'].length - 1; i++) {
            ...
            //
            let catidNew =  catid+"-row";
            $("div[data-id= '"+catidNew+"']").append(chalwrap);
            //
        };
